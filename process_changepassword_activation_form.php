    ...
    if (mysqli_select_db($dbc, 'MFT_DB')) {
        ...
        if ($result) {
            ...
            if (password_verify($oldpass, $row[0])) {
                ...
                $error = $query2;
                if ($result2) {
                    header("location: activation_sucess.php");
                } else {
...
<head>
<meta charset="utf-8">
<title>Money Finance Tracker -- Process Change Password On Activation</title>
...
?>
	...
	<h1>Money Finance Tracker -- Process Change Password On Activation</h1>
